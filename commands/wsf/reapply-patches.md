...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/wsf-local-patches" ]; then
    ...
    PATCHES_DIR="$HOME/.claude/wsf-local-patches"
  fi
fi
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${WSF_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
