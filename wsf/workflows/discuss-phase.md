...
<required_reading>
@~/.claude/wsf/references/domain-probes.md
@~/.claude/wsf/references/gate-prompts.md
@~/.claude/wsf/references/universal-anti-patterns.md
</required_reading>
...
AGENT_SKILLS_ADVISOR=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" agent-skills wsf-advisor 2>/dev/null)
```
...
- Read and execute @~/.claude/wsf/workflows/discuss-phase-power.md end-to-end
...
```bash
TODO_MATCHES=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" todo match-phase "${PHASE_NUMBER}")
```
...
1. Check for USER-PROFILE.md:
   ```bash
   PROFILE_PATH="$HOME/.claude/wsf/USER-PROFILE.md"
   ```
...
3. Resolve model for advisor agents:
   ```bash
   ADVISOR_MODEL=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" resolve-model wsf-advisor-researcher --raw)
   ```
...
2. For EACH user-selected gray area, spawn a Task() in parallel:
   ...
   Task(
     prompt="First, read @~/.claude/agents/wsf-advisor-researcher.md for your role and instructions.
...
```bash
MAX_PASSES=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" config-get workflow.max_discuss_passes 2>/dev/null || echo "3")
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" state record-session \
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" --cwd "${project_root}" commit "docs(state): record phase ${PHASE} context session" --files .planning/STATE.md
```
</step>
...
**Full step-by-step instructions:** @~/.claude/wsf/workflows/discuss-phase-power.md
