...
如果 NO_GIT：静默运行 `git init`。WSF 项目总是有自己的仓库。
</git_check>
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs: initialize [project-name] ([N] phases)" --files .planning/
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-PLAN.md .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md
```
...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/
```
