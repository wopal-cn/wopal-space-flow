...
模型配置控制每个 WSF 代理使用哪个 Claude 模型。这允许平衡质量和 token 消耗。
...
| wsf-nyquist-auditor | sonnet | sonnet | haiku |
...
**为什么 wsf-planner 使用 Opus？**
规划涉及架构决策、目标分解和任务设计。这是模型质量影响最大的地方。

**为什么 wsf-executor 使用 Sonnet？**
...
**为什么 wsf-codebase-mapper 使用 Haiku？**
只读探索和模式提取。不需要推理，只需从文件内容输出结构化结果。

**为什么用 `inherit` 而不是直接传递 `opus`？**
Claude Code 的 `"opus"` 别名映射到特定模型版本。组织可能阻止旧版 opus 而允许新版。WSF 为 opus 级代理返回 `"inherit"`，使其使用用户在会话中配置的任何 opus 版本。这避免了版本冲突和静默回退到 Sonnet。