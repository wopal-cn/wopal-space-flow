...
function cmdInitExecutePhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    executor_model: resolveModelInternal(cwd, 'wsf-executor'),
    verifier_model: resolveModelInternal(cwd, 'wsf-verifier'),
...
function cmdInitPlanPhase(cwd, phase, raw, options = {}) {
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'wsf-phase-researcher'),
    planner_model: resolveModelInternal(cwd, 'wsf-planner'),
    checker_model: resolveModelInternal(cwd, 'wsf-plan-checker'),
...
function cmdInitNewProject(cwd, raw) {
  ...
  const braveKeyFile = path.join(homedir, '.wsf', 'brave_api_key');
  const hasBraveSearch = !!(process.env.BRAVE_API_KEY || fs.existsSync(braveKeyFile));

  // Detect Firecrawl API key availability
  const firecrawlKeyFile = path.join(homedir, '.wsf', 'firecrawl_api_key');
  const hasFirecrawl = !!(process.env.FIRECRAWL_API_KEY || fs.existsSync(firecrawlKeyFile));

  // Detect Exa API key availability
  const exaKeyFile = path.join(homedir, '.wsf', 'exa_api_key');
  ...
  const result = {
    // Models
    researcher_model: resolveModelInternal(cwd, 'wsf-project-researcher'),
    synthesizer_model: resolveModelInternal(cwd, 'wsf-research-synthesizer'),
    roadmapper_model: resolveModelInternal(cwd, 'wsf-roadmapper'),
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const result = {
    // Models
    mapper_model: resolveModelInternal(cwd, 'wsf-codebase-mapper'),
...
function cmdInitManager(cwd, raw) {
  ...
  if (!fs.existsSync(paths.roadmap)) {
    error('No ROADMAP.md found. Run /wsf-new-milestone first.');
  }
  if (!fs.existsSync(paths.state)) {
    error('No STATE.md found. Run /wsf-new-milestone first.');
  }
  ...
  for (const phase of phases) {
    ...
    if (phase.disk_status === 'planned' && phase.deps_satisfied) {
      recommendedActions.push({
        ...
        command: `/wsf-execute-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/wsf-plan-phase ${phase.number}`,
      });
      ...
      recommendedActions.push({
        ...
        command: `/wsf-discuss-phase ${phase.number}`,
      });
    }
  }
  ...
  const sanitizeFlags = (raw) => {
    ...
    if (!safe) {
      process.stderr.write(`wsf-tools: warning: manager.flags contains invalid tokens, ignoring: ${val}\n`);
      return '';
    }
    return val;
  };
...
function cmdInitNewWorkspace(cwd, raw) {
  ...
  const defaultBase = path.join(homedir, 'wsf-workspaces');
 ...
 *
 * Reads `config.agent_skills[agentType]` and resolves skill paths using:
 * 1. Absolute paths: use directly
 * 2. Relative paths: search project root first, then workspace root
 * 
 * Returns a formatted `<agent_skills>` block or empty string if no skills are configured.
 *
 * @param {object} config - Loaded project config
 * @param {string} agentType - The agent type (e.g., 'wsf-executor', 'wsf-planner')
...
function buildAgentSkillsBlock(config, agentType, projectRoot) {
  ...
  // Detect workspace root (used for relative path fallback)
  ...
  for (const skillPath of skillPaths) {
    ...
    // Basic traversal check for relative paths
    if (!path.isAbsolute(skillPath) && skillPath.includes('..')) {
      process.stderr.write(`[agent-skills] WARNING: Skipping traversal path "${skillPath}"\n`);
      continue;
    }
    ...
    if (path.isAbsolute(skillPath)) {
      ...
      skillDir = skillPath;
      foundBase = null; // No base directory for absolute paths
    } else {
      // Relative path: try project root first, then workspace root
      const projectSkillDir = path.join(projectRoot, skillPath);
      if (fs.existsSync(path.join(projectSkillDir, 'SKILL.md'))) {
        skillDir = skillPath;
        foundBase = projectRoot;
      } else if (workspaceRoot) {
        const workspaceSkillDir = path.join(workspaceRoot, skillPath);
        if (fs.existsSync(path.join(workspaceSkillDir, 'SKILL.md'))) {
          skillDir = skillPath;
          foundBase = workspaceRoot;
        }
      }
    }

    // For absolute paths, check SKILL.md existence directly
    if (path.isAbsolute(skillPath)) {
      ...
      // Relative path found in project or workspace
      validPaths.push(skillPath);
    } else {
      // Relative path not found anywhere
      process.stderr.write(`[agent-skills] WARNING: Skill not found at "${skillPath}/SKILL.md" — skipping\n`);
      continue;
    }
  }
 ...
 * Used by workflows: SKILLS=$(node "$TOOLS" agent-skills wsf-executor 2>/dev/null)
 */
