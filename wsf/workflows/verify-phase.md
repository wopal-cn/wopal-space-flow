...
<required_reading>
@~/.claude/wsf/references/verification-patterns.md
@~/.claude/wsf/templates/verification-report.md
</required_reading>
...
```bash
INIT=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" init phase-op "${PHASE_ARG}")
...
See ~/.claude/wsf/templates/verification-report.md for complete template.
</step>
