...
```bash
node "$HOME/.claude/wsf/bin/wsf-tools.cjs" history-digest
```
...
@~/.claude/wsf/references/thinking-models-planning.md
...
```bash
VALID=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" verify plan-structure "$PLAN_PATH")
```
