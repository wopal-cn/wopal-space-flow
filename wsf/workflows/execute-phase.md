...
@~/.claude/wsf/references/context-budget.md
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/wsf/workflows/execute-plan.md` **inline**
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Task(
     ...
     prompt="
       ...
       @~/.claude/wsf/references/checkpoints.md
       @~/.claude/wsf/references/tdd.md
       </execution_context>
...
Read and follow `~/.claude/wsf/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.
