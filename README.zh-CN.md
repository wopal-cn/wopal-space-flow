...
所以我做了 WSF。复杂性在系统内部，不在你的工作流里。幕后是上下文工程、XML 提示格式、子代理编排、状态管理；你看到的是几个真能工作的命令。
...
WSF 解决的就是这个问题。它是让 Claude Code 变得可靠的上下文工程层。你只要描述想法，系统会自动提取它需要知道的一切，然后让 Claude Code 去干活。
...
- Codex：`$wsf-help`
- Cline：WSF 通过 `.clinerules` 安装 — 检查 `.clinerules` 是否存在

> [!NOTE]
> Claude Code 2.1.88+ 和 Codex 以 skill 形式安装（`skills/wsf-*/SKILL.md`）。Cline 使用 `.clinerules`。安装器会自动处理所有格式。
...
```bash
# Claude Code
npx wsf-cc --claude --global   # 安装到 ~/.claude/
npx wsf-cc --claude --local    # 安装到 ./.claude/

# OpenCode
npx wsf-cc --opencode --global # 安装到 ~/.config/opencode/

# Gemini CLI
npx wsf-cc --gemini --global   # 安装到 ~/.gemini/

# Kilo
npx wsf-cc --kilo --global     # 安装到 ~/.config/kilo/
npx wsf-cc --kilo --local      # 安装到 ./.kilo/

# Codex
npx wsf-cc --codex --global    # 安装到 ~/.codex/
npx wsf-cc --codex --local     # 安装到 ./.codex/

# Copilot
npx wsf-cc --copilot --global  # 安装到 ~/.github/
npx wsf-cc --copilot --local   # 安装到 ./.github/

# Cursor CLI
npx wsf-cc --cursor --global   # 安装到 ~/.cursor/
npx wsf-cc --cursor --local    # 安装到 ./.cursor/

# Antigravity
npx wsf-cc --antigravity --global # 安装到 ~/.gemini/antigravity/
npx wsf-cc --antigravity --local  # 安装到 ./.agent/

# Augment
npx wsf-cc --augment --global     # 安装到 ~/.augment/
npx wsf-cc --augment --local      # 安装到 ./.augment/

# Trae
npx wsf-cc --trae --global     # 安装到 ~/.trae/
npx wsf-cc --trae --local      # 安装到 ./.trae/

# Cline
npx wsf-cc --cline --global       # 安装到 ~/.cline/
npx wsf-cc --cline --local        # 安装到 ./.clinerules

# 所有运行时
npx wsf-cc --all --global      # 安装到所有目录
```
...
WSF 的设计目标是无摩擦自动化。运行 Claude Code 时建议使用：
...
> [!TIP]
> 这才是 WSF 的预期用法。连 `date` 和 `git commit` 都要来回确认 50 次，整个体验就废了。
...
> **已经有现成代码库？** 先运行 `/wsf-map-codebase`。它会并行拉起多个代理分析你的技术栈、架构、约定和风险点。之后 `/wsf-new-project` 就会真正“理解”你的代码库，提问会聚焦在你打算新增的部分，规划时也会自动加载你的现有模式。
...
如果一切通过，就进入下一步；如果哪里坏了，你不需要手动 debug，只要重新运行 `/wsf-execute-phase`，执行它自动生成的修复计划即可。
...
如果你希望在讨论阶段更快收集信息，可以用 `/wsf-discuss-phase <n> --batch`，一次回答一小组问题，而不是逐个问答。

每个阶段都会得到你的输入（discuss）、充分研究（plan）、干净执行（execute）和人工验证（verify）。上下文始终保持新鲜，质量也能持续稳定。

当所有阶段完成后，`/wsf-complete-milestone` 会归档当前里程碑并打 release tag。

接着用 `/wsf-new-milestone` 开启下一个版本。它和 `new-project` 流程相同，只是面向你现有的代码库。你描述下一步想构建什么，系统研究领域、梳理需求，再产出新的路线图。每个里程碑都是一个干净周期：定义 → 构建 → 发布。
...
快速模式保留 WSF 的核心保障（原子提交、状态跟踪），但路径更短：
...
| 命令 | 作用 |
|------|------|
| `/wsf-new-project [--auto]` | 完整初始化：提问 → 研究 → 需求 → 路线图 |
| `/wsf-discuss-phase [N] [--auto] [--analyze]` | 在规划前收集实现决策（`--analyze` 增加权衡分析） |
| `/wsf-plan-phase [N] [--auto] [--reviews]` | 为某个阶段执行研究 + 规划 + 验证（`--reviews` 加载代码库审查结果） |
| `/wsf-execute-phase <N>` | 以并行 wave 执行全部计划，完成后验证 |
| `/wsf-verify-work [N]` | 人工用户验收测试 ¹ |
| `/wsf-ship [N] [--draft]` | 从已验证的阶段工作创建 PR，自动生成 PR 描述 |
| `/wsf-fast <text>` | 内联处理琐碎任务——完全跳过规划，立即执行 |
| `/wsf-next` | 自动推进到下一个逻辑工作流步骤 |
| `/wsf-audit-milestone` | 验证里程碑是否达到完成定义 |
| `/wsf-complete-milestone` | 归档里程碑并打 release tag |
| `/wsf-new-milestone [name]` | 开始下一个版本：提问 → 研究 → 需求 → 路线图 |
| `/wsf-milestone-summary` | 从已完成的里程碑产物生成项目概览，用于团队上手 |
| `/wsf-forensics` | 对失败或卡住的工作流进行事后调查 |

### 工作流（Workstreams）

| 命令 | 作用 |
|------|------|
| `/wsf-workstreams list` | 显示所有工作流及其状态 |
| `/wsf-workstreams create <name>` | 创建命名空间工作流，用于并行里程碑工作 |
| `/wsf-workstreams switch <name>` | 切换当前活跃工作流 |
| `/wsf-workstreams complete <name>` | 完成并合并工作流 |

### 多项目工作区

| 命令 | 作用 |
|------|------|
| `/wsf-new-workspace` | 创建隔离工作区，包含仓库副本（worktree 或 clone） |
| `/wsf-list-workspaces` | 显示所有 WSF 工作区及其状态 |
| `/wsf-remove-workspace` | 移除工作区并清理 worktree |

### UI 设计

| 命令 | 作用 |
|------|------|
| `/wsf-ui-phase [N]` | 为前端阶段生成 UI 设计合约（UI-SPEC.md） |
| `/wsf-ui-review [N]` | 对已实现前端代码进行 6 维视觉审计 |
...
| 命令 | 作用 |
|------|------|
| `/wsf-map-codebase` | 在 `new-project` 前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|------|------|
| `/wsf-add-phase` | 在路线图末尾追加 phase |
| `/wsf-insert-phase [N]` | 在 phase 之间插入紧急工作 |
| `/wsf-remove-phase [N]` | 删除未来 phase，并重编号 |
| `/wsf-list-phase-assumptions [N]` | 在规划前查看 Claude 打算采用的方案 |
| `/wsf-plan-milestone-gaps` | 为 audit 发现的缺口创建 phase |

### 代码质量

| 命令 | 作用 |
|------|------|
| `/wsf-review` | 对当前阶段或分支进行跨 AI 同行评审 |
| `/wsf-pr-branch` | 创建过滤 `.planning/` 提交的干净 PR 分支 |
| `/wsf-audit-uat` | 审计验证债务——找出缺少 UAT 的阶段 |

### 积压

| 命令 | 作用 |
|------|------|
| `/wsf-plant-seed <idea>` | 将想法存入积压停车场，留待未来里程碑 |

### 会话

| 命令 | 作用 |
|------|------|
| `/wsf-pause-work` | 在中途暂停时创建交接上下文（写入 HANDOFF.json） |
| `/wsf-resume-work` | 从上一次会话恢复 |
| `/wsf-session-report` | 生成会话摘要，包含已完成工作和结果 |

### 工具

| 命令 | 作用 |
|------|------|
| `/wsf-settings` | 配置模型 profile 和工作流代理 |
| `/wsf-set-profile <profile>` | 切换模型 profile（quality / balanced / budget / inherit） |
| `/wsf-add-todo [desc]` | 记录一个待办想法 |
| `/wsf-check-todos` | 查看待办列表 |
| `/wsf-debug [desc]` | 使用持久状态进行系统化调试 |
| `/wsf-do <text>` | 将自由文本自动路由到正确的 WSF 命令 |
| `/wsf-note <text>` | 零摩擦想法捕捉——追加、列出或提升为待办 |
| `/wsf-quick [--full] [--discuss] [--research]` | 以 WSF 保障执行临时任务（`--full` 增加计划检查和验证，`--discuss` 先补上下文，`--research` 在规划前先调研） |
| `/wsf-health [--repair]` | 校验 `.planning/` 目录完整性，带 `--repair` 时自动修复 |
| `/wsf-stats` | 显示项目统计——阶段、计划、需求、git 指标 |
| `/wsf-profile-user [--questionnaire] [--refresh]` | 从会话分析生成开发者行为档案，用于个性化响应 |
...
WSF 将项目设置保存在 `.planning/config.json`。你可以在 `/wsf-new-project` 时配置，也可以稍后通过 `/wsf-settings` 修改。完整的配置 schema、工作流开关、git branching 选项以及各代理的模型分配，请查看[用户指南](docs/USER-GUIDE.md#configuration-reference)。
...
可以用 `/wsf-settings` 开关这些项，也可以在单次命令里覆盖：
...
| `git.phase_branch_template` | string | `wsf/phase-{phase}-{slug}` | phase 分支模板 |
| `git.milestone_branch_template` | string | `wsf/{milestone}-{slug}` | milestone 分支模板 |

**策略说明：**
- **`none`**：直接提交到当前分支（WSF 默认行为）
- **`phase`**：每个 phase 创建一个分支，在 phase 完成时合并
- **`milestone`**：整个里程碑只用一个分支，在里程碑完成时合并

在里程碑完成时，WSF 会提供 squash merge（推荐）或保留历史的 merge 选项。
...
WSF 的代码库映射和分析命令会读取文件来理解你的项目。**包含机密信息的文件应当加入 Claude Code 的 deny list**：
...
> [!IMPORTANT]
> WSF 内建了防止提交 secrets 的保护，但纵深防御依然是最佳实践。第一道防线应该是直接禁止读取敏感文件。
...
- 检查文件是否存在于 `~/.claude/commands/wsf/`（全局）或 `./.claude/commands/wsf/`（本地）
- 对 Codex，检查 skills 是否存在于 `~/.codex/skills/wsf-*/SKILL.md`（全局）或 `./.codex/skills/wsf-*/SKILL.md`（本地）

**命令行为不符合预期？**
- 运行 `/wsf-help` 确认安装成功
- 重新执行 `npx wsf-cc` 进行重装
...
这会移除所有 WSF 命令、代理、hooks 和设置，但会保留你其他配置。

---

## 社区移植版本

OpenCode、Gemini CLI、Kilo 和 Codex 现在都已经通过 `npx wsf-cc` 获得原生支持。

这些社区移植版本曾率先探索多运行时支持：

| Project | Platform | Description |
|---------|----------|-------------|
| [wsf-opencode](https://github.com/rokicool/wsf-opencode) | OpenCode | 最初的 OpenCode 适配版本 |
| wsf-gemini (archived) | Gemini CLI | uberfuzzy 制作的最初 Gemini 适配版本 |
