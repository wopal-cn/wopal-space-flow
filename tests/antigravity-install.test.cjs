/**
 * WSF Tools Tests - Antigravity Install Plumbing
 *
...
describe('convertClaudeToAntigravityContent', () => {
  ...
  describe('local install path replacements', () => {
    test('replaces ~/.claude/ with .agent/ for local installs', () => {
      const input = 'See ~/.claude/wsf/';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/wsf/'), result);
      assert.ok(!result.includes('~/.claude/'), result);
    });
    ...
    test('replaces .claude/ with .agent/', () => {
      const input = 'node .claude/hooks/wsf-statusline.js';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('.agent/hooks/wsf-statusline.js'), result);
      assert.ok(!result.includes('.claude/'), result);
    });
  });

  describe('command name conversion', () => {
    test('converts /wsf-command to /wsf-command', () => {
      const input = 'Run /wsf-new-project to start';
      const result = convertClaudeToAntigravityContent(input, true);
      assert.ok(result.includes('/wsf-new-project'), result);
      assert.ok(!result.includes('wsf:'), result);
    });

    test('converts all wsf- references', () => {
      const input = '/wsf-plan-phase and /wsf-execute-phase';
      const result = convertClaudeToAntigravityContent(input, false);
      assert.ok(result.includes('/wsf-plan-phase'), result);
      assert.ok(result.includes('/wsf-execute-phase'), result);
    });
  });
...
description: Initialize a new WSF project with requirements and roadmap
...
Initialize new project at ~/.claude/wsf/workflows/new-project.md
`;

  test('produces name and description only in frontmatter', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, 'wsf-new-project', false);
    assert.ok(result.startsWith('---\n'), result);
    assert.ok(result.includes('name: wsf-new-project'), result);
    assert.ok(result.includes('description: Initialize a new WSF project'), result);
  ...
  test('uses provided skillName for name field', () => {
    const result = convertClaudeCommandToAntigravitySkill(claudeCommand, 'wsf-custom-name', false);
    assert.ok(result.includes('name: wsf-custom-name'), result);
  });

  test('converts wsf- command references in body', () => {
    const content = `---
name: test
description: test skill
---
Run /wsf-new-project to get started.
`;
    const result = convertClaudeCommandToAntigravitySkill(content, 'wsf-test', false);
    assert.ok(result.includes('/wsf-new-project'), result);
    assert.ok(!result.includes('wsf:'), result);
  });
  ...
  test('returns unchanged content when no frontmatter', () => {
    ...
    const result = convertClaudeCommandToAntigravitySkill(noFm, 'wsf-test', false);
...
Execute plans from ~/.claude/wsf/workflows/execute-phase.md
`;

  test('preserves name and description', () => {
    const result = convertClaudeAgentToAntigravityAgent(claudeAgent, false);
    assert.ok(result.includes('name: wsf-executor'), result);
    assert.ok(result.includes('description: Executes WSF plans'), result);
  });
  ...
  test('uses global path for global installs', () => {
    ...
    assert.ok(result.includes('~/.gemini/antigravity/wsf/'), result);
  });
...
describe('copyCommandsAsAntigravitySkills', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('wsf-ag-test-');
    srcDir = path.join(tmpDir, 'commands', 'wsf');
...
`);
  ...
  test('creates skills directory', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'wsf', false);
    assert.ok(fs.existsSync(skillsDir));
  });

  test('creates one skill directory per command with SKILL.md', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'wsf', false);
    const skillDir = path.join(skillsDir, 'wsf-new-project');
  ...
  test('handles subdirectory commands with prefixed names', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'wsf', false);
    const subSkillDir = path.join(skillsDir, 'wsf-subdir-sub-command');
  ...
  test('SKILL.md has minimal frontmatter (name + description only)', () => {
    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'wsf', false);
    const content = fs.readFileSync(path.join(skillsDir, 'wsf-new-project', 'SKILL.md'), 'utf8');
    assert.ok(content.includes('name: wsf-new-project'), content);
  ...
  test('SKILL.md body has paths converted for local install', () => {
    ...
    const content = fs.readFileSync(path.join(skillsDir, 'wsf-new-project', 'SKILL.md'), 'utf8');
    // wsf- → wsf- conversion
    assert.ok(!content.includes('wsf:'), content);
  });

  test('removes old wsf-* skill dirs before reinstalling', () => {
    // Create a stale skill dir
    const staleDir = path.join(skillsDir, 'wsf-old-skill');
  ...
  test('does not remove non-wsf skill dirs', () => {
    // Create a non-WSF skill dir
    const otherDir = path.join(skillsDir, 'my-custom-skill');
    fs.mkdirSync(otherDir, { recursive: true });

    copyCommandsAsAntigravitySkills(srcDir, skillsDir, 'wsf', false);

    assert.ok(fs.existsSync(otherDir), 'non-WSF skill dir should be preserved');
  });
});
...
describe('writeManifest (Antigravity)', () => {
  ...
  beforeEach(() => {
    tmpDir = createTempDir('wsf-manifest-ag-');
    // Create minimal structure
    const skillsDir = path.join(tmpDir, 'skills', 'wsf-help');
    fs.mkdirSync(skillsDir, { recursive: true });
    fs.writeFileSync(path.join(skillsDir, 'SKILL.md'), '---\nname: wsf-help\ndescription: Help\n---\n');
    const wsfDir = path.join(tmpDir, 'wsf');
    fs.mkdirSync(wsfDir, { recursive: true });
    fs.writeFileSync(path.join(wsfDir, 'VERSION'), '1.0.0');
    const agentsDir = path.join(tmpDir, 'agents');
    fs.mkdirSync(agentsDir, { recursive: true });
    fs.writeFileSync(path.join(agentsDir, 'wsf-executor.md'), '---\nname: wsf-executor\n---\n');
  });
  ...
  test('manifest includes skills in skills/ directory', () => {
    writeManifest(tmpDir, 'antigravity');
    const manifest = JSON.parse(fs.readFileSync(path.join(tmpDir, 'wsf-file-manifest.json'), 'utf8'));
