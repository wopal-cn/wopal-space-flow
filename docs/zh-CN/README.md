...
所以我开发了 WSF。复杂性在系统内部，不在你的工作流里。幕后是：上下文工程、XML 提示格式、子代理编排、状态管理。你看到的是：几个命令，用就完了。
...
WSF 解决了这个问题。它是让 Claude Code 变得可靠的上下文工程层。描述你的想法，让系统提取它需要知道的一切，然后让 Claude Code 开始工作。
...
WSF 设计为无摩擦自动化。运行 Claude Code 时使用：
...
> [!TIP]
> 这是 WSF 的预期使用方式 —— 停下来 50 次批准 `date` 和 `git commit` 会失去意义。
...
> **已有代码？** 先运行 `/wsf-map-codebase`。它会生成并行代理分析你的技术栈、架构、约定和关注点。然后 `/wsf-new-project` 就了解你的代码库了 —— 问题聚焦在你正在**添加**什么，规划会自动加载你的模式。
...
如果一切通过，继续。如果有东西坏了，不用手动调试 —— 只需再次运行 `/wsf-execute-phase`，使用它创建的修复计划。
...
如果你想在讨论期间更快速地输入，使用 `/wsf-discuss-phase <n> --batch` 一次回答一组小问题，而不是一个一个来。使用 `--chain` 可以自动链式执行从讨论到规划+执行，中间不停顿。

每个阶段都会获得你的输入（讨论）、适当的研究（规划）、干净的执行（执行）和人工验证（验证）。上下文保持新鲜。质量保持高水平。

当所有阶段完成后，`/wsf-complete-milestone` 归档里程碑并标记发布。

然后 `/wsf-new-milestone` 开始下一个版本 —— 与 `new-project` 相同的流程，但针对你现有的代码库。你描述接下来想构建什么，系统研究领域，你界定需求范围，它创建新的路线图。每个里程碑是一个干净的周期：定义 → 构建 → 发布。
...
快速模式给你 WSF 保证（原子提交、状态跟踪）和更快的路径：
...
**`--full` 标志：** 启用所有阶段 —— 讨论 + 研究 + 计划检查 + 验证。快速任务形式的完整 WSF 管道。
...
| `/wsf-discuss-phase [N] [--auto] [--chain] [--power]` | 在规划前捕获实现决策（`--chain` 自动链式执行规划+执行，`--power` 文件批量输入） |
| `/wsf-plan-phase [N] [--auto]` | 阶段的研究 + 规划 + 验证 |
| `/wsf-execute-phase <N>` | 在并行波次中执行所有计划，完成后验证 |
| `/wsf-verify-work [N]` | 手动用户验收测试 ¹ |
| `/wsf-audit-milestone` | 验证里程碑达到了其完成定义 |
| `/wsf-complete-milestone` | 归档里程碑，标记发布 |
...
| 命令 | 作用 |
|---------|--------------|
| `/wsf-map-codebase` | 在 new-project 之前分析现有代码库 |

### 阶段管理

| 命令 | 作用 |
|---------|--------------|
| `/wsf-add-phase` | 向路线图追加阶段 |
| `/wsf-insert-phase [N]` | 在阶段之间插入紧急工作 |
| `/wsf-remove-phase [N]` | 删除未来阶段，重新编号 |
| `/wsf-list-phase-assumptions [N]` | 规划前查看 Claude 的预期方法 |
| `/wsf-plan-milestone-gaps` | 创建阶段以填补审计发现的差距 |
| `/wsf-autonomous [--from N] [--to N] [--only N]` | 自主执行所有剩余阶段（`--to N` 执行到阶段 N 停止，`--only N` 只执行单个阶段） |
| `/wsf-analyze-dependencies` | 检测阶段间依赖关系并建议 ROADMAP.md 的 `Depends on` 条目 |

### 会话

| 命令 | 作用 |
|---------|--------------|
| `/wsf-pause-work` | 阶段中途停止时创建交接 |
| `/wsf-resume-work` | 从上次会话恢复 |

### 工具

| 命令 | 作用 |
|---------|--------------|
| `/wsf-settings` | 配置模型配置文件和工作流代理 |
| `/wsf-set-profile <profile>` | 切换模型配置文件（quality/balanced/budget） |
| `/wsf-add-todo [desc]` | 捕获想法留待后用 |
| `/wsf-check-todos` | 列出待处理事项 |
| `/wsf-debug [desc] [--diagnose]` | 带持久状态的系统化调试（`--diagnose` 仅诊断不修复） |
| `/wsf-quick [--full] [--discuss] [--research]` | 用 WSF 保证执行临时任务（`--full` 启用全部阶段，`--discuss` 先收集上下文，`--research` 规划前调查方法） |
| `/wsf-health [--repair]` | 验证 `.planning/` 目录完整性，用 `--repair` 自动修复 |
...
WSF 在 `.planning/config.json` 中存储项目设置。在 `/wsf-new-project` 期间配置或稍后用 `/wsf-settings` 更新。完整配置模式、工作流开关、git 分支选项和每个代理的模型分解，请参阅[用户指南](USER-GUIDE.md#配置参考)。
...
| `git.phase_branch_template` | 字符串 | `wsf/phase-{phase}-{slug}` | 阶段分支模板 |
| `git.milestone_branch_template` | 字符串 | `wsf/{milestone}-{slug}` | 里程碑分支模板 |

**策略：**
- **`none`** —— 提交到当前分支（默认 WSF 行为）
- **`phase`** —— 每个阶段创建一个分支，阶段完成时合并
- **`milestone`** —— 为整个里程碑创建一个分支，完成时合并

在里程碑完成时，WSF 提供 squash 合并（推荐）或带历史合并。
...
WSF 的代码库映射和分析命令读取文件以了解你的项目。**保护包含密钥的文件**，将它们添加到 Claude Code 的拒绝列表：
...
> [!IMPORTANT]
> WSF 包含内置保护以防止提交密钥，但纵深防御是最佳实践。拒绝读取敏感文件作为第一道防线。
...
**安装后找不到命令？**
- 重启运行时以重新加载命令/技能
- 验证文件是否存在于 `~/.claude/commands/wsf/`（全局）或 `./.claude/commands/wsf/`（本地）
- 对于 Codex，验证技能是否存在于 `~/.codex/skills/wsf-*/SKILL.md`（全局）或 `./.codex/skills/wsf-*/SKILL.md`（本地）

**命令没有按预期工作？**
- 运行 `/wsf-help` 验证安装
- 重新运行 `npx wsf-cc` 重新安装
...
这删除所有 WSF 命令、代理、钩子和设置，同时保留你的其他配置。

---

## 社区移植

OpenCode、Gemini CLI、Kilo 和 Codex 现在通过 `npx wsf-cc` 原生支持。

这些社区移植开创了多运行时支持：

| 项目 | 平台 | 描述 |
|---------|----------|-------------|
| [wsf-opencode](https://github.com/rokicool/wsf-opencode) | OpenCode | 原始 OpenCode 适配 |
| wsf-gemini (已归档) | Gemini CLI | 由 uberfuzzy 开发的原始 Gemini 适配 |
