...
```bash
RESEARCHER_MODEL=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" resolve-model wsf-phase-researcher --raw)
```

## 1. Validate Phase

```bash
PHASE_INFO=$(node "$HOME/.claude/wsf/bin/wsf-tools.cjs" roadmap get-phase "${phase_number}")
```
